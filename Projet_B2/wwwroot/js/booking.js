document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadSpaces(){
    // Load rooms (persisted) so users can book rooms
    ...
    // After loading spaces, fetch existing bookings for the selected space/date
    spaceSel.addEventListener('change', ()=> fetchBookedSlots());
  }
  ...
  async function fetchBookedSlots(){
    ...
    items.forEach(it=>{
      ...
      const start = it.startHour ?? new Date(it.start).getHours();
      const hours = it.hours ?? Math.max(1, Math.round((new Date(it.end)-new Date(it.start))/3600000));
      li.textContent = `${it.date || ''} ${start}:00 for ${hours}h — ${it.status || ''}`;
      ul.appendChild(li);
    });
    out.appendChild(ul);
  }
  ...
  function createBookedList(){
    ...
    el.id = 'bookedList';
    const form = document.querySelector('#bookingContainer form');
    form.appendChild(el);
    return el;
  }
  ...
  loadSpaces().then(()=> fetchBookedSlots());
